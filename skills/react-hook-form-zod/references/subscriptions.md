...
- If React Compiler is enabled, check actual compilation/skip behavior and compatibility of the installed RHF APIs. `watch` uses internal mutable state: memoizing a read merely because its function reference is stable can freeze the displayed value with manual or automatic memoization. Compiler can skip incompatible consumers to preserve correctness. Connect reactive consumers through a supported subscription such as `useWatch` and verify that new input reaches them; this is not a blanket ban on `watch`, a requirement to replace a correct skipped path, or a substitute for event-time `getValues`.
...
- [React Compiler incompatible-library](https://react.dev/reference/eslint-plugin-react-hooks/lints/incompatible-library).

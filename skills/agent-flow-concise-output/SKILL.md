...
For PR descriptions and summaries, always read `write-for-work` and apply its PR descriptions mode, including when drafting rather than shortening. For other output, apply it when a trivial cut is insufficient. Preserve every parser-owned marker, verdict, status, and `next_command` value exactly as emitted by the active workflow.

...
- 판정 불가는 차단이 아니다. 셸 파싱 실패나 cwd 미선언은 통과시키고(파괴 명령만 예외), 대신 lifecycle 명령(`status`/`continue`/`run`/`start`)은 **어떤 상태에서도** 통과시킨다. 경계가 만든 교착의 해제 명령이 그 경계 뒤에 있으면 탈출구가 0이 된다. leader가 정상 fast-forward된 뒤의 stale baseline은 `agent-flow host-session rebind --session-key <key> --expected-old-head <old> --expected-new-head <new>`로만 푼다 — `status` 재실행 같은 부수적 행위로는 절대 열리지 않는다.

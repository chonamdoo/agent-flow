...
tools/skill-noop/       ← A/B harness that measures whether a skill line changes review outcomes. Not shipped
```
...
- `tools/skill-noop/` stays outside `scripts/`. `scripts/` is a shipped asset tree — it is in `package.json.files`, in `RECORDED_KIT_ASSET_TREES` (`lib/installer-shared.mjs`), and in `KIT_SOURCE_DIGEST_ROOTS` (`src/agent_flow/core/kit_digest.py`, `bin/agent-flow-kit.mjs`) — so a harness there installs its measurement corpus into every user project, and each run rewrites a digest root and makes `warn_if_installed_kit_is_stale` fire in every installed project. The harness writes `results-n<reps>-<providers>.jsonl` next to its own case, which is why its tree must not be one the installer reads.

...
1. `agent-flow run "<task>"`가 Run을 만들고 git repo에서는 `~/.agent-flow/worktrees/<repo-id>/feat-<slug>/` worktree에서 시작한다.
...
- **Worktree**: 독립 변경을 만들기 위한 git worktree. `~/.agent-flow/worktrees/<repo-id>/feat-<slug>/`를 기본 자리로 쓰고(leader 밖에 두어 leader IDE watcher와 worker 변경을 격리한다) 브랜치는 `feat/<slug>`를 기본값으로 둔다. 예전 자리 `<repo>.worktrees/<name>/`, `.agent-flow/worktrees/<name>/`의 checkout은 계속 인식된다.

...
function validateSkillMetadataPayload(payload, documents) {
  ...
  const references = (value) => Array.isArray(value) && value.every((entry) =>
    typeof entry === "string"
    || (record(entry) && typeof entry.path === "string"
      && Object.keys(entry).every((key) => ["path", "pathGlobs"].includes(key))
      && (!Object.hasOwn(entry, "pathGlobs")
        || (strings(entry.pathGlobs) && entry.pathGlobs.length > 0))));
  const metadata = (value) => record(value)
    && ["requires", "dependencies", "architecture_modes"].every(
      (key) => !Object.hasOwn(value, key) || strings(value[key]),
    )
    && (!Object.hasOwn(value, "requires_docs") || references(value.requires_docs))

...
export function resolveRuntimeSkillPlan(index, {
  ...
  requiredSkills = [],
  indexRoot = null,
  activeHost = process.env.AGENT_FLOW_ACTIVE_HOST || process.env.AGENT_FLOW_HOST || "",
  home = os.homedir(),
} = {}) {
  ...
  const managedHostOnly = runtimeManagedHostOnlySkillCatalog(index, indexRoot);
  const normalizedActiveHost = String(activeHost || "").trim().toLowerCase();
  for (const name of [...required].sort(compareCodePoints)) {
    const skill = byName.get(name);
    const resolution = compatibility.resolve(name);
    if (!skill) {
      const hostOnlyCatalog = managedHostOnly.get(name);
      if (hostOnlyCatalog) {
        const local = localManagedHostOnlySkill(
          name,
          hostOnlyCatalog,
          normalizedActiveHost,
          home,
        );
        if (local.skill) {
          if (resolution.capabilities.length > 0) {
            local.skill.capabilities = [...resolution.capabilities];
          }
          skills.push(local.skill);
          continue;
        }
        missing.add(name);
        for (const requested of requestsByCanonical.get(name) || [name]) {
          resolutionErrors.set(`${requested}\0${local.diagnostic.reason}`, {
            ...local.diagnostic,
            requested,
...
function skillCatalogsFromProfileYaml(text) {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    const hostsSection = line.match(/^  (hosts|source_hosts):\s*$/);
    if (hostsSection && source) {
      const catalog = ensureCatalog();
      if (hostsSection[1] === "source_hosts" && catalog.source_hosts === null) {
        catalog.source_hosts = Object.create(null);
      }
      mode = hostsSection[1];
      continue;
    }
    ...
    if (host && source && (mode === "hosts" || mode === "source_hosts")) {
      ensureCatalog()[mode][portableSkillCasefold(host[1])] = stripQuotes(host[2].trim());
      continue;
    }
...
function runtimeManagedHostOnlySkillCatalog(index, indexRoot) {
  const result = new Map();
  if (typeof indexRoot !== "string" || indexRoot.length === 0) return result;
  const profilesRoot = [
    path.join(indexRoot, ".agent-flow", "profiles"),
    path.join(indexRoot, "profiles"),
  ].find((candidate) => fs.existsSync(candidate));
  if (!profilesRoot) return result;
  for (const profile of uniqueLogicalNames(index?.selection?.profiles || []).sort(compareCodePoints)) {
    const profilePath = path.join(profilesRoot, `${profile}.yaml`);
    if (!fs.existsSync(profilePath)) continue;
    ...
    for (const [catalogName, catalog] of Object.entries(catalogs)) {
      if (catalog.install_policy !== "never" || catalog.active_host_only !== true) continue;
      for (const name of [...catalog.implementation, ...catalog.review]) {
        const canonical = portableSkillCasefold(name);
        const record = {
          catalog: catalogName,
          source: catalog.source,
          hosts: { ...(catalog.source_hosts ?? catalog.hosts) },
        };
        const previous = result.get(canonical);
        if (previous && JSON.stringify(previous) !== JSON.stringify(record)) {
          throw new Error(`conflicting managed host-only skill catalog: ${canonical}`);
        }
        result.set(canonical, record);
      }
    }
  }
  return result;
...
function activeHostSkillRoot(activeHost, home) {
  const resolvedHome = path.resolve(String(home || os.homedir()));
  if (activeHost === "claude") return path.join(resolvedHome, ".claude", "skills");
  if (activeHost === "codex") return path.join(resolvedHome, ".codex", "skills");
  if (activeHost === "omp") return path.join(resolvedHome, ".omp", "agent", "skills");
  return null;
}

function localManagedHostOnlySkill(name, catalog, activeHost, home) {
  const source = catalog.source;
  const hostPath = catalog.hosts[activeHost];
  const root = activeHostSkillRoot(activeHost, home);
  if (!root || typeof hostPath !== "string" || hostPath.length === 0) {
    return {
      diagnostic: {
        reason: activeHost ? "unsupported_active_host" : "active_host_required",
        canonical: name,
        source: typeof source === "string" ? source : null,
        message: activeHost
          ? `active host ${activeHost} cannot load local ${catalog.catalog}: ${name}`
          : `active host is required to load local ${catalog.catalog}: ${name}`,
        repairable: false,
      },
    };
  }
  const skillRoot = path.join(root, name);
  const document = path.join(skillRoot, "SKILL.md");
  try {
    const canonicalRoot = fs.realpathSync(root);
    const skillRootMetadata = fs.lstatSync(skillRoot);
    const documentMetadata = fs.lstatSync(document);
    if (
      skillRootMetadata.isSymbolicLink()
      || !skillRootMetadata.isDirectory()
      || documentMetadata.isSymbolicLink()
      || !documentMetadata.isFile()
    ) throw new Error("unsafe host skill path");
    const canonicalSkillRoot = fs.realpathSync(skillRoot);
    const canonicalDocument = fs.realpathSync(document);
    const relative = path.relative(canonicalRoot, canonicalSkillRoot);
    const documentRelative = path.relative(canonicalSkillRoot, canonicalDocument);
    if (
      relative === ""
      || relative === ".."
      || relative.startsWith(`..${path.sep}`)
      || path.isAbsolute(relative)
      || documentRelative !== "SKILL.md"
    ) throw new Error("unsafe host skill path");
  } catch {
    return {
      diagnostic: {
        ...
        canonical: name,
        source,
        message: `missing local ${catalog.catalog}: ${name}`,
        repairable: false,
      },
    };
  }
...
export function requiredReviewSkillsFromProfileYaml(profilePath) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (blockListIndent !== null) {
      ...
      if (indent > blockListIndent && item) {
        const name = stripQuotes(item[1].trim());
        if (name) values.push(name);
        continue;
      }
      if (line.trim() && indent <= blockListIndent) blockListIndent = null;
    }
    ...
    if (/^\s+skills:\s*$/.test(line)) blockListIndent = indent;
  }
  validateSafeNames(values, "profile skill");

...
export function canonicalizeInstallSelectionCompatibility(
  ...
  const concreteSources = [
    [path.join(kitRoot, "skills"), kitRoot],
    [path.join(projectRoot, ".agent-flow", "local-skills"), projectRoot],
  ];
  if (!sameDirectory(projectRoot, kitRoot)) {
    concreteSources.push([path.join(projectRoot, "skills"), projectRoot]);
  }
  for (const [base, authority] of concreteSources) {
    if (!fs.existsSync(base)) continue;
    for (const skill of projectSkillCatalog(base, authority)) {
...
export function validateLockedSkillTrees(kitRoot) {
  ...
  for (const name of lockedNames) {
    ...
    if (hashSkillTree(skillRoot, { authorityRoot: kitRoot, skillName: name }) !== projectTreeHashes[name]) {
...
export function validateAndroidOfficialLock(kitRoot, official) {
  ...
  for (const name of snapshotNames) {
    ...
    const skillRoot = path.join(kitRoot, "skills", name);
    if (
      hashSkillTree(skillRoot, { authorityRoot: kitRoot, skillName: name })
...
export function readSkillDocument(
  ...
  if (initial.isSymbolicLink() || !initial.isFile() || !isSingleLink(initial)) {
    throw skillDocumentResolutionError(
      documentPath,
      skillName,
...
function readStableSkillTreeFile(file) {
  const noFollow = fs.constants.O_NOFOLLOW || 0;
  const nonBlocking = fs.constants.O_NONBLOCK || 0;
  ...
  if (!initial.isFile() || !isSingleLink(initial) || !sameFilesystemIdentity(file.metadata, initial)) {
  ...
  for (const profile of touchedProfiles) {
    ...
    for (const route of routing.profiles?.[profile]?.skill_routes || []) {
      ...
      for (const name of logicalNameValues(route.skills)) {
        if (profileCatalog.has(name)) rawRequired.add(name);
      }
    }
  }

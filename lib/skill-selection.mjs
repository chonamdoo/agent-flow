...
export function resolveInstallSelection({ args, detectedProfile, kitRoot, projectRoot = kitRoot }) {
  ...
  const autoProfile = requestedProfiles.length === 0 && (
    PROFILE_SKILLS.has(detectedProfile)
    || fs.existsSync(path.join(kitRoot, "profiles", `${detectedProfile}.yaml`))
  )
    ? [detectedProfile]
    : [];
...
export function mergeInstallSelectionWithPrevious(selection, previousIndex, kitRoot, projectRoot = kitRoot) {
  ...
  if (
    ...
    && previousSelection.mode === "filtered"
    && previousSelection.profile_selection === "explicit"
  ) {
    const previousProfiles = uniqueLogicalNames(previousSelection.profiles || []);
    const previousExplicitSkills = new LogicalNameSet(previousSelection.explicit_skills || []);
    for (const skill of selection.explicitSkills || []) previousExplicitSkills.add(skill);
  ...
  if (previousSelection.mode === "filtered") {
    if (previousSelection.profile_selection === "explicit") {
...
export function resolveProfileSkillSources({
  ...
  const regularRoots = new LogicalNameSet(
    [...pendingNames].filter((name) => !automaticRoots.has(name)),
  );
  ...
  while (true) {
    ...
    for (const source of ["project-local", "project"]) {
      ...
      assertSkillDocumentLineLimit(candidate.root, name);
    ...
    if (!selected && previous && snapshotExists) {
      ...
      const snapshotLineLimitError = skillDocumentLineLimitError(snapshot.root, name);
      const managedSnapshotIsUnchanged = previous?.tree_hash === snapshotHash
        && REPLACEABLE_SNAPSHOT_SOURCES.has(previous?.source);
      if (["host-bootstrap", "shared"].includes(previous?.source)) {
        ...
        const sourceFailure = !original
          ? `pinned external skill source is unavailable: ${name}`
          : original.tree_hash !== previous.tree_hash
            ? `pinned external skill source changed: ${name}`
            : null;
        if (sourceFailure) {
          const bundled = snapshotLineLimitError && managedSnapshotIsUnchanged
            ? inspectExternalSkillCandidate(candidates.get("bundled"), name, rejectedExternal)
            : null;
      ...
      if ((previous?.source === "bundled" || snapshotLineLimitError) && managedSnapshotIsUnchanged) {
        const bundled = inspectExternalSkillCandidate(candidates.get("bundled"), name, rejectedExternal);
        if (bundled && bundled.tree_hash !== snapshotHash) {
          selected = { ...bundled, replace_existing: true };
        }
      }
      if (!selected && snapshotLineLimitError) {
        throw snapshotLineLimitError;
      }
...
export function discoverAutomaticExternalSkillNames({
  ...
  for (const source of roots) {
    for (const name of validatedExternalSkillCatalog(source)) names.add(name);
  }
...
export function validateAndroidOfficialLock(kitRoot, official) {
  ...
  for (const name of snapshotNames) {
    ...
    assertSkillSourceProvenance(kitRoot, skillRoot);
    assertSkillDocumentLineLimit(skillRoot, name);
...
function assertSkillDocumentLineLimit(root, name) {
  const error = skillDocumentLineLimitError(root, name);
  if (error) throw error;
}

function skillDocumentLineLimitError(root, name) {
  const skillPath = path.join(root, "SKILL.md");
  const lineCount = fs.readFileSync(skillPath, "utf8").split(/\r?\n/).length;
  if (lineCount > MAX_SKILL_MD_LINES) {
    return new Error(
      `${skillPath}: ${lineCount} lines; max is ${MAX_SKILL_MD_LINES}, split progressive references for ${name}`,
    );
  }
  return null;
}
...
export function loadProfileRouting(kitRoot) {
  ...
  for (const [profile, config] of Object.entries(routing.profiles)) {
    ...
    const installed = new LogicalNameSet(profileSkillsFromSource(kitRoot, profile));
    ...
    for (const route of config.skill_routes || []) {
      ...
      for (const skill of route.skills) {
        ...
        if (!installed.has(logicalSkill) || !conditionalNames.has(logicalSkill)) {
          throw new Error(`unavailable routed skill: ${profile}/${skill}`);
        }
        routed.add(logicalSkill);
      }
    }

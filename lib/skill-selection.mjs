...
export function resolveRuntimeSkillPlan(index, {
  ...
  if (!CODE_SKILL_PHASES.has(phaseId) && phaseRequired.size === 0) {
  ...
  const missingProfiles = [...touchedProfiles]
    ...
    .sort(compareCodePoints);
  const required = new LogicalNameSet(phaseRequired);
  if (CODE_SKILL_PHASES.has(phaseId)) required.add("code-generation-discipline");

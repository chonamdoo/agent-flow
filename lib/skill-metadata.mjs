...
export function recordSkillLinkReceipt(link, skill, previousIndex) {
  ...
  const installedContentDigest = ["linked", "copied"].includes(link.status)
...
export function captureLegacySkillCopyReceipts(root, previousIndex) {
  ...
  for (const link of previousIndex.links) {
    if (!link || link.status !== "copied" || link.installedContentDigest) continue;
...
export function skippedSkillCopy(link) {
  const unverified = !link.installedContentDigest && ["copied", "skipped-unverified-copy"].includes(link.status);

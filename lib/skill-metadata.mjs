...
import { arrayValue, ensureChildPath, pathHasSymlink, safeSkillName, uniqueStrings } from "./installer-shared.mjs";
...
export function skillCopyMatchesReceipt(directory, ...digests) {
  const { digest } = observeSkillContent(directory);
  return Boolean(digest) && digests.some((expected) => expected && expected === digest);
}

function isCopiedSkillStatus(status) {
  return status === "copied" || (typeof status === "string" && status.startsWith("copied:"));
}

export function recordSkillLinkReceipt(link, skill, previousIndex) {
  const previous = previousIndex?.links?.find((item) => item.name === link.name && item.host === link.host);
  const installedContentDigest = link.status === "linked" || isCopiedSkillStatus(link.status)
    ? skill.observedContentDigest : previous?.installedContentDigest;
  return installedContentDigest ? { ...link, installedContentDigest } : link;
}

export function captureLegacySkillCopyReceipts(root, previousIndex) {
  if (!Array.isArray(previousIndex?.links) || !Array.isArray(previousIndex?.skills)) return;
  for (const link of previousIndex.links) {
    if (!link || !isCopiedSkillStatus(link.status) || link.installedContentDigest) continue;
    const skill = previousIndex.skills.find((item) => item?.name === link.name);
    if (typeof skill?.path !== "string" || typeof link.path !== "string") continue;
    const source = path.dirname(path.join(root, skill.path));
    const target = path.join(root, link.path);
    ensureChildPath(root, source);
    ensureChildPath(root, target);
    if (pathHasSymlink(root, source) || pathHasSymlink(root, target)) continue;
    const { digest } = observeSkillContent(source);
    // Compare against the pre-upgrade content, never a SKILL.md-only hash.
    if (digest && skillCopyMatchesReceipt(target, digest)) {
      link.installedContentDigest = digest;
    }
  }
}

export function skippedSkillCopy(link) {
  const unverified = !link.installedContentDigest
    && (isCopiedSkillStatus(link.status) || link.status === "skipped-unverified-copy");
  const status = unverified ? "skipped-unverified-copy" : "skipped-user-modified";
  if (unverified) {
    console.warn(`kept unverified copied skill: ${link.path}; inspect and back up its contents before using --force-managed`);
  }
  return { ...link, status };
}
...
function governanceValueKind(frontmatter, key) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (let nested = index + 1; nested < lines.length; nested += 1) {
      ...
      kind = /^\s/.test(lines[nested]) ? "structured" : "blank";
      break;
    }
  }
  return kind;

...
// 그 아래 hook들이 통째로 죽었다. 사본을 두고 대조하는 대신 한 벌만 둔다.
//
// 루트 `AGENTS.md` ↔ `CLAUDE.md`를 전체 파일로 미러링하던 hook은 없다. OMP는 루트
// `CLAUDE.md`를 애초에 읽지 않는다 — `claude` discovery provider의 경로는
// `.claude/CLAUDE.md`와 `~/.claude/CLAUDE.md`뿐이고, 루트 `AGENTS.md`는 `agents-md`
// provider가 읽는다. 그 미러의 전제는 "두 파일이 같은 본문의 사본이다"였고, 지금은
// `CLAUDE.md`가 `AGENTS.md`를 가리키는 한 줄 포인터라 맞출 대상이 없다.
...
const HOOK_LAUNCHER = path.join(INSTALL_ROOT, ".agent-flow", "bin", "agent-flow-hook");
...
export default function agentFlowHooks(pi) {
  ...
  pi.on("tool_result", async (event, ctx) => {
    ...
    if (COMMAND_TOOL_RE.test(toolName)) {
      ...
      const cwd = event?.input?.cwd;
      // Host 전용 경로 표기의 실제 해석은 완료 이벤트에 없으므로 추측하지 않는다.
      const hasFilesystemCwd = typeof cwd === "string" && cwd.length > 0
        && !/^(?:[~@:]|[a-z][a-z\d+.-]*:)|:\/\/|local:\/|[\\\u00a0\u2000-\u200a\u202f\u205f\u3000]/i.test(cwd)
        && !/^\/+$/.test(cwd);
      const recorderPayload = hasFilesystemCwd
        ? { ...payload, cwd: path.isAbsolute(cwd) ? cwd : path.resolve(payload.cwd, cwd) }
        : payload;
      await runHook("record-command-run.py", recorderPayload, ctx);
  ...
  pi.on("session_shutdown", async (event, ctx) => {
    const payload = { ...hookPayload(event, ctx), hook_event_name: "session_shutdown" };
    const result = await runHook("show-phase-status.sh", payload, ctx);
...
function spawnHook(scriptName, scriptPath, input, cwd) {
  return new Promise((resolve) => {
    // 하드코딩 /usr/bin/python3·/bin/bash 대신 install이 pin한 인터프리터를 태우는
    // portable launcher로 실행한다. host가 /usr/bin/python3을 직접 execve하지 못하는
    // 자리에서도 hook이 죽지 않는다 (hook exec EPERM).
    const proc = spawn(HOOK_LAUNCHER, [scriptPath], { cwd, stdio: ["pipe", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
    let settled = false;
    const finish = (result) => {

...
// ROOT는 "확장 파일이 install root 안에 있다"가 아니라 "install root를 찾는다"로
// 푼다. 확장을 worktree로 복사하면 2단 상향은 <worktree>가 되고, 거기엔
// .agent-flow/scripts/hooks가 없다 -> HOOK_DIR가 빈 경로가 되어 guard가 전부
// 무음이 된다. 복사된 확장도 실제로 guard를 돌려야 하므로 leader의 hook
// 스크립트를 찾아간다.
//
// Python LEAKY_GIT_ENV_VARS(src/agent_flow/core/worktree_isolation.py)와 같은 목록이다.
// ambient GIT_COMMON_DIR 하나만 남아도 rev-parse가 남의 repo를 가리킨다.
...
// linked worktree의 <checkout>/.git은 "gitdir: <leader>/.git/worktrees/<name>" 한 줄이
// 든 파일이다. 그 한 줄이면 leader가 결정되므로 git을 부를 필요가 없다 — timeout,
// safe.directory 거절, PATH에 git 부재가 전부 이 경로에서는 문제가 되지 않는다.
function gitdirLeaderRoot(start) {
  let current = start;
  while (true) {
    const pointer = path.join(current, ".git");
    try {
      if (fs.statSync(pointer).isFile()) {
        const line = fs.readFileSync(pointer, "utf8").trim();
        if (line.startsWith("gitdir:")) {
          const target = line.slice("gitdir:".length).trim();
          if (target) {
            // <leader>/.git/worktrees/<name> -> <leader>
            const admin = path.resolve(current, target);
            const leader = path.dirname(path.dirname(path.dirname(admin)));
            if (path.basename(path.dirname(path.dirname(admin))) === ".git") {
              return leader;
            }
          }
        }
...
function gitLeaderRoot(start) {
  const env = { ...process.env };
  for (const name of GIT_DISCOVERY_ENV) {
    delete env[name];
  }
  // 이 호출은 모듈 최상단(const ROOT = ...)에서 돈다. git이 응답하지 않으면
  // 확장 로드 자체가 매달려 host가 뜨지 않는다. 형제 구현(hook 스크립트 timeout=5,
  // parity 30_000, kit safeSpawnSync)과 같이 상한을 둔다.
  ...
  // --git-common-dir은 cwd 기준 상대경로를 낼 수 있어 start 기준으로 푼다.
  const resolved = path.resolve(start, commonDir);
...
// kit.json만으로는 후보가 될 수 없다. hook 스크립트가 없는 루트를 ROOT로 잡으면
// HOOK_DIR 전체가 빈 경로가 되어 모든 hook이 "실행조차 못 함"으로 떨어진다.
// 설치 흔적이 아니라 "여기서 hook을 돌릴 수 있다"를 기준으로 고른다.
function isExtensionRoot(dir) {
  const base = path.join(dir, ".agent-flow");
  if (!fs.existsSync(path.join(base, "kit.json"))) {
    return false;
  }
  try {
    return fs.statSync(path.join(base, "scripts", "hooks")).isDirectory();
  } catch {
    return false;
  }
}

// leader 우선. 조상 탐색만 쓰면 $HOME/.agent-flow/kit.json이 실제 repo leader를
// 가리고, leader 우선만 쓰면 worktree-local kit.json이 자기를 자가인증한다.
function resolveExtensionRoot(start) {
  // gitdir 파일이 먼저다. 사실상 모든 linked worktree가 여기서 결정되므로 git 하위
  // 프로세스는 거의 돌지 않고, 그 실패 모드(timeout·safe.directory·PATH)도 사라진다.
  for (const candidate of [gitdirLeaderRoot(start), gitLeaderRoot(start)]) {
    if (candidate && isExtensionRoot(candidate)) {
      return candidate;
    }
  }
  let current = start;
  while (true) {
    ...
    const parent = path.dirname(current);
    if (parent === current) {
      // 여기까지 오면 HOOK_DIR가 실재하지 않는다 = 이 checkout의 모든 guard가 조용히
      // 통과한다. 통째 무음이면 버그로 부를 단서가 없으므로 한 번은 드러낸다.
      console.error(
        "agent-flow: managed hooks not found from " + start
        + "; expected " + path.join(start, ".agent-flow", "scripts", "hooks")
        + " — enforcement hooks will not run in this checkout"
      );
      return start;
    }
    current = parent;
...
const ROOT = resolveExtensionRoot(path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", ".."));
...
async function runHook(scriptName, payload, ctx) {
  ...
  // "스크립트가 없다"와 "스크립트가 돌아서 실패했다"는 다른 사건이다. 부재를
  // block으로 취급하면 ROOT 오판(예: leader 디렉터리가 옮겨져 gitdir가 dangling)
  // 한 번에 Write/Edit/MultiEdit/Bash가 전면 차단되고, 사용자는 그 세션에서 복구
  // 명령조차 돌릴 수 없다. 설치본의 hook 부재는 run-start 게이트(hook_integrity)가
  // 잡는 사안이므로 여기서는 non-block으로 통과시킨다.
  ...
  // 스크립트가 실제로 돌아서 0이 아니면 그대로 block이다 - 부재와 이 경우를
  // 뭉치면 guard가 무력화된다.

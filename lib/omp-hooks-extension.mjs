...
function isExactSpecApproval(prompt) {
  return prompt === "승인";
}

async function runSpecUserPromptHooks(prompt, event, ctx, provenancePolicy) {
  if (provenancePolicy !== "allow-unknown" && provenancePolicy !== "require-user") {
    reportApprovalDiagnostics(ctx, {
      reason: "invalid OMP SPEC approval provenance policy; approval was not recorded",
    });
    return;
  }
  const provenance = sessionProvenance(ctx);
  if (provenance === "subagent" || (
    provenance === "unknown" && provenancePolicy === "require-user"
  )) {
    reportApprovalDiagnostics(ctx, {
      reason: provenance === "subagent"
        ? "ignored SPEC approval from OMP subagent session"
        : "cannot verify top-level OMP session; SPEC approval was not recorded",
    });
    return;
  }
  const payload = {
  ...
  // 두 hook이 exit 0이어도 binding 무효 사유를 버리면 승인이 조용히 무시되어 보인다.
  reportApprovalDiagnostics(ctx, await runHook("prepare-spec-user-prompt.py", payload, ctx));
  reportApprovalDiagnostics(ctx, await runHook("confirm-spec-user-prompt.py", payload, ctx));
}


function sessionProvenance(ctx) {
  try {
    ...
    const canonical = (value) => {
      let existing = path.resolve(ctx?.cwd || ROOT, value);
      const missing = [];
      while (true) {
        try {
          ...
          if (parent === existing) {
            return path.resolve(ctx?.cwd || ROOT, value);
          }
    ...
    // OMP 17.2.1은 user session에는 자신의 artifacts dir을, Task subagent에는
    // 부모 artifacts dir을 주입한다. 존재하는 가장 가까운 조상을 먼저 정규화해
    // 아직 생성되지 않은 user artifacts dir도 symlink 경계에서 같은 값으로 비교한다.
    const canonicalSession = canonical(sessionFile);
    const canonicalArtifacts = canonical(artifactsDir);
    if (samePath(
      canonicalSession.slice(0, -".jsonl".length),
      canonicalArtifacts,
    )) {
      return "user";
    }
    return isAncestorOrSame(canonicalArtifacts, canonicalSession)
      ? "subagent"
      : "unknown";
  } catch {
    return "unknown";
...
export default function agentFlowHooks(pi) {
  ...
  pi.on("input", async (event, ctx) => {
    ...
    if (!isExactSpecApproval(prompt)) {
      return;
    }
    // v17.2.1은 before_agent_start에서 session path provenance를 검증한다. 그 API가
    // 없는 구형 interactive host만 input에서 처리해 같은 queued prompt를 두 번 승인하지 않는다.
    const manager = ctx?.sessionManager;
    if (
      typeof manager?.getSessionFile === "function"
      && typeof manager?.getArtifactsDir === "function"
    ) {
      return;
    }
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    // 비대화형 fallback에는 source marker가 없으므로 v17.2.1 session path로 증명한다.
...
// 승인 hook이 올린 사유를 사용자에게 보이는 자리로 옮긴다. UI가 있으면 알림,
// 없으면 logger/stderr. 성공 경로에서는 hook이 아무것도 안 내므로 잡음이 없다.
function reportApprovalDiagnostics(ctx, result) {
  const reason = String(result?.reason || "").trim();
  if (!reason) {
    return;
  }
  const message = "agent-flow: " + reason;

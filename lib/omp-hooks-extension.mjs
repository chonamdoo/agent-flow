...
function resolveInstallRoot(start) {
  ...
  for (;;) {
    ...
    // 프로젝트를 삼킨다 — cli.py의 _managed_worktree_context와 같은 경계다.
...
async function runHook(scriptName, payload, ctx) {
  ...
  if (!hookScriptIsInstalled(scriptPath)) {
    // 스크립트 부재는 설치·해석 문제이지 정책 위반이 아니다. 이것을 block으로
    // 올리면 spawn 실패 하나가 그 세션의 모든 bash/write를 막는다. 대신 사유를
    // 낸다 — 조용히 삼키면 이 버그가 그대로 돌아온다.

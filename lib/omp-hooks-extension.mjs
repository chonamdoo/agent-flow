...
export default function agentFlowHooks(pi) {
  ...
  pi.on("input", async (event, ctx) => {
    ...
    // 이 두 hook은 exit 0으로 끝나도 사유를 stderr로 올린다(binding 무효 등).
    // 여기서 버리면 사용자에게는 승인이 조용히 무시된 것으로만 보인다.
...
function sessionIdentity(event, ctx) {
  ...
  // 세션 식별자가 없어 direct는 항상 비고, 이 getter가 유일한 출처다. 여기서 빈
  // 문자열이 나가면 승인 hook과 worktree binding hook이 session_id 없음으로 조용히
  // 끝난다 — 채팅 승인이 무시되던 실제 원인이 이 자리였다.
  // 같은 실측에서 task 도구로 띄운 subagent의 이벤트는 부모와 다른 id를 받았다
  // (한 프로세스 안에서 부모 019fb170-24f0... / subagent 019fb170-6c1e...,
  // 부모의 대기 중인 tool_call과 겹친 시점에도 갈렸다). 즉 관측된 범위에서 이
  // getter는 프로세스 전역이 아니라 이벤트가 속한 세션의 id를 준다. 그 밖의
  // 세션 종류(재개, 중첩 subagent)는 재보지 않았다.
  try {
    const managed = ctx?.sessionManager?.getSessionId?.();
    return typeof managed === "string" ? managed.trim() : "";
  } catch {
    return "";
  }
}

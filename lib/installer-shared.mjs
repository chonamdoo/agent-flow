...
// 제자리 truncate는 중단·동시 install 때 다음 실행이 읽을 수 없는 JSON을 남긴다.
// 같은 디렉터리의 새 파일을 flush한 뒤 rename해 이전 판본 또는 새 판본만 보이게 한다.
...
// 거부하므로(가로채기 방지) umask에 맡기면 환경에 따라 조용히 죽는다. 제자리
// truncate로 쓰면 install 중에 도는 hook이 반쪽 파일을 해시하거나 exec해 그
// 프롬프트의 승인이 조용히 사라지므로 같은 디렉터리에 쓰고 rename한다. symlink를
...
function recordBootstrapReceipt(root, label, block) {
  ...
  fs.writeFileSync(receipt, `${JSON.stringify({ blocks }, null, 2)}\n`, "utf8");
}

...
function readBootstrapReceipts(root) {
  const payload = readJsonIfExists(path.join(root, BOOTSTRAP_RECEIPT_RELATIVE));
...
function recordBootstrapReceipt(root, label, block) {
  ...
  const blocks = { ...readBootstrapReceipts(root), [label]: bootstrapBlockDigest(block) };
...
export function bootstrapBlockIsOurs(root, label) {
  ...
  return block !== null && readBootstrapReceipts(root)[label] === bootstrapBlockDigest(block);
}
...
export function upsertRootBootstrapBlock(root, label, block, { force = false } = {}) {
  ...
  const recorded = readBootstrapReceipts(root)[label];
  const userEdited = typeof recorded === "string" && recorded !== bootstrapBlockDigest(existing);
  if (userEdited && !force) {
    return { label, status: "kept-user-edited", backup: null };
  }

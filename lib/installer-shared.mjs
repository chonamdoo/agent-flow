...
export function docsIndexBlock(root) {
  const grouped = collectDocsIndexEntries(path.join(root, DOCS_INDEX_ROOT), DOCS_INDEX_ROOT);
  if (grouped.length === 0) {
  ...
  for (const { dir, files } of grouped) {
    ...
    if (used + line.length > DOCS_INDEX_MAX_CHARS) {
  ...
  if (omitted > 0) {
    lines.push(`|+${omitted} more under ${DOCS_INDEX_ROOT}/ — 목록이 상한을 넘어 잘렸다. 직접 훑어라.`);
  }
...
// 프로젝트가 있고, 따라가면 순환에 빠지거나 남의 트리를 계약에 싣는다.
function collectDocsIndexEntries(absDir, relDir) {
  ...
  for (const dirent of dirents.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    if (dirent.isDirectory()) {
      nested.push(...collectDocsIndexEntries(path.join(absDir, dirent.name), `${relDir}/${dirent.name}`));
    } else if (dirent.isFile() && dirent.name.endsWith(".md")) {
      files.push(dirent.name);
    }
  }
  return [...(files.length > 0 ? [{ dir: relDir, files }] : []), ...nested];
}

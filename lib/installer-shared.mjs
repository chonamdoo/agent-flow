...
//
// PyYAML은 user-site에만 있는 배치가 흔하므로(`/usr/bin/python3`이 그렇다) `-I`는
// 못 쓴다. 대신 `-E`로 환경변수 주입만 끊고 관리 runtime 경로는 argv로 넘긴다.
...
export function resolveManagedPython() {
  ...
  for (const candidate of candidates) {
    // 실행 시점과 **같은 플래그**로 검사한다. `-E` 없이 통과한 인터프리터는
    // PYTHONPATH로만 yaml을 얻고 있을 수 있어 launcher에서 다시 죽는다.
    const probe = spawnSync(
      candidate,
      ["-E", "-c", "import yaml, sys; sys.stdout.write(sys.executable)"],
...
// 내용은 프로젝트 경로에 의존하지 않는다(root는 `$0`에서 유도). 인터프리터만
// 박히므로 같은 머신의 두 프로젝트는 같은 바이트를 갖고, digest 하나로 대조된다.
export function projectLauncherSource(pythonPath) {
...
set -u
# 로더 주입 경로는 \`-E\`가 막지 못한다. 승인 hook이 타는 유일한 실행 경로라 끊는다.
unset PYTHONPATH PYTHONHOME PYTHONSTARTUP LD_PRELOAD LD_LIBRARY_PATH DYLD_INSERT_LIBRARIES DYLD_LIBRARY_PATH DYLD_FRAMEWORK_PATH
launcher_dir=\${0%/*}
...
exec "$python" -E -c 'import sys; sys.path.insert(0, sys.argv.pop(1)); from agent_flow.cli import main; raise SystemExit(main())' "$runtime" "$@"
`;
}

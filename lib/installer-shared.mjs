...
export const RECORDED_KIT_ASSET_TREES = Object.freeze([
  { src: "skills", dest: ".agent-flow/skills", skip: isBundledSkillManifest },
...
export function isRecordedKitAsset(relativeToRoot) {
  ...
  return RECORDED_KIT_ASSET_TREES.some((tree) => {
    if (!label.startsWith(`${tree.dest}/`)) {
      return false;
    }
    return !tree.skip?.(label.slice(tree.dest.length + 1));
  });
}
...
export function readHookSettings(settingsPath) {
  ...
  try {
    ...
    const backupPath = nextFreeBackupPath(`${settingsPath}.bak`, content);
    if (backupPath !== null) {
      fs.copyFileSync(settingsPath, backupPath);
    }
    throw new Error(
      `could not parse ${settingsPath}; preserved the original`
      + (backupPath === null ? "" : ` and copied ${backupPath}`),
    );
  }
}

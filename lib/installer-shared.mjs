...
// 루트 `AGENTS.md`와 `CLAUDE.md`는 같은 블록을 받는다. 예전에는 label마다 템플릿이
// 따로 있어 CLAUDE.md만 포인터였고, 그래서 host에 따라 로드되는 계약이 달랐다:
// Codex와 OMP는 `AGENTS.md`를 직접 읽으므로 그 포인터가 아무 일도 하지 않았다.
// 파일을 한 벌로 두면 사본이 갈라질 자리가 없어 label별 템플릿이 필요 없다.
export const BOOTSTRAP_TEMPLATE_FILE = "AGENTS.md.template";

// 블록 **밖**은 프로젝트 소유 산문이고, 그것의 자리는 `AGENTS.md`다. Claude CLI는 루트
// `CLAUDE.md`만 자동 로드하므로 그 산문이 Claude와 `multi-review`의 Claude reviewer에게
// 닿는 경로는 이 import 한 줄뿐이다. `@path`는 파일 전체를 끌어오니 Claude는 블록을 두 번
// 보게 된다(33줄). 그 중복을 받는 이유는 반대쪽 손실이 크기 때문이다: import가 없으면
// 프로젝트가 블록 밖에 적은 규칙 — 크기 제한이 없다 — 이 host에 따라 조용히 사라진다.
//
// `AGENTS.md`에는 넣지 않는다. 자기 자신을 import하는 줄이고, Claude 외 host에서는
// 뜻 없는 텍스트다. 그래서 두 루트 블록의 유일한 차이가 이 줄이다.
const CLAUDE_ROOT_IMPORT_LINES = [
  "",
  "이 파일 밖의 프로젝트 규칙은 `AGENTS.md`에 있다. 아래 줄은 Claude Code의 import 지시자로 그것을 끌어온다 — Claude CLI는 루트 `CLAUDE.md`만 자동 로드하고 루트 `AGENTS.md`는 로드하지 않는다. 지우면 Claude reviewer가 블록 밖 규칙을 받지 못한다.",
  "",
  "@AGENTS.md",
];

export function rootBootstrapBlock(label, templateText) {
  if (label !== "CLAUDE.md") return templateText;
  const end = "<!-- agent-flow:end -->";
  const at = templateText.lastIndexOf(end);
  if (at === -1) {
    throw new Error(`bootstrap template has no ${end} marker`);
  }
  return `${templateText.slice(0, at)}${CLAUDE_ROOT_IMPORT_LINES.join("\n")}\n${templateText.slice(at)}`;
}

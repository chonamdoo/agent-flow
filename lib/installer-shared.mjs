...
import { skillSummaryFromMarkdown } from "./frontmatter.mjs";
...
export function skillIndexBlock(root) {
  ...
  if (onDemand.length > 0) {
    // 이름만으로는 "scope가 걸리는가"를 판단할 재료가 없다. 한 문장이면 충분하고,
    // 규칙은 Python `skill_summary`와 같아야 프롬프트와 인덱스가 갈리지 않는다.
    lines.push("|on-demand:");
    for (const skill of onDemand) {
      const name = String(skill.name);
      const summary = skillSummaryForIndex(root, skill);
...
function skillSummaryForIndex(root, skill) {
  const relative = typeof skill.path === "string" && skill.path
    ? skill.path
    : path.join(".agent-flow", "skills", String(skill.name), "SKILL.md");
  try {
    return skillSummaryFromMarkdown(fs.readFileSync(path.join(root, relative), "utf8"));
  } catch {
    return "";
  }
}

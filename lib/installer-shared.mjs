...
export function upsertGitExclude(root, entries) {
  ...
  // linked worktree의 `.git`은 파일이다. install은 그런 checkout에서 이미 멈추므로
  // 여기 오면 디렉터리여야 하고, 아니면 우리가 아는 자리가 아니니 건드리지 않는다.
...
export function upsertRootBootstrapBlock(root, label, block, { force = false } = {}) {
  ...
  let backup = null;
  if (userEdited) {
    // 되찾기 전에 사용자가 쓴 것을 남긴다. 관리 영역 안이라 프로젝트 트리를 어지럽히지
    // 않고, hook script prune과 같은 `.removed` 규칙을 따른다 — 두 번째 force가 첫
    // 사본을 덮지 않도록 내용이 다르면 digest를 붙인 사본을 따로 남긴다.
...
export function reportRootBootstrapBlocks(results) {
  for (const result of results) {
    ...
    } else if (result.status === "restored-forced") {

...
//      사용자가 좁혀 둔 설정 권한이 install마다 풀리면 안 된다.
//   3. 신규 파일이다: 0o666으로 열고 커널의 umask가 정하게 둔다. 여기서 mode를
//      명시하거나 chmod를 걸면 umask 077 환경이 요구한 0600이 0644로 넓어진다.
// symlink는 물려받을 대상이 아니다 - rename이 갈아 끼우는 것은 링크 자신이고,
// 링크의 mode(대개 0777)를 새 파일에 심으면 아무도 요구하지 않은 권한 확대가 된다.
...
export function removeCodexBroadTrustState(root) {
  ...
  if (next !== current) {
    // rename은 링크가 아니라 링크 자신을 갈아 끼운다. 사용자가 `~/.codex/config.toml`을
    // 다른 곳으로 심링크해 뒀다면 그 링크가 일반 파일로 바뀌어 원본이 갱신되지 않는다.
    // 우리 파일이 아니므로 따라간 실체를 쓴다.
    atomicWriteFileSync(fs.realpathSync(configPath), next.endsWith("\n") ? next : `${next}\n`);
  }
}

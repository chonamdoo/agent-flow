...
// lib/yaml-runtime.mjs
var yaml_runtime_exports = {};
__export(yaml_runtime_exports, {
  mergeProfilePayloads: () => mergeProfilePayloads,
  parseInstalledProfileYaml: () => parseInstalledProfileYaml,
  parseWorkflowYaml: () => parseWorkflowYaml,
  parseYamlMapping: () => parseYamlMapping,
  renderProfilePromptBlock: () => renderProfilePromptBlock
});
module.exports = __toCommonJS(yaml_runtime_exports);
var import_yaml = __toESM(require_dist(), 1);
var ARCHITECTURE_PHASES = /* @__PURE__ */ new Set([
  "design",
  "ddd-design",
  "architecture-review",
  "slice-plan",
...
var PROFILE_PROMPT_EXCLUDED_FIELDS = /* @__PURE__ */ new Set([
  "skills",
  "android_skills",
  "android_ecosystem_skills",
  "chrisbanes_skills"
]);
function parseWorkflowYaml(text, name, source = `${name}.yaml`) {
  ...
  for (const phase of phases) {
    ...
    if ((phase.multi_review || phase.id === "multi-review" || phase.id === "gates") && (!phase.routes || Object.keys(phase.routes).length === 0)) {
...
function parseInstalledProfileYaml(text, profileId, source = `${profileId}.yaml`) {
...
function mergeProfilePayloads(profiles, primaryProfile = null) {
  const deduped = [];
...
function renderProfilePromptBlock(profileId, profile, phaseId) {
  const snapshot = profilePromptProjection(profile, phaseId);
  if (Object.keys(snapshot).length === 0) return "";
  const yaml = (0, import_yaml.stringify)(snapshot, {
    indent: 2,
    indentSeq: false,
    lineWidth: 0,
    schema: "yaml-1.1"
...
function validateInstalledProfileShape(payload, profileId) {
  if (Object.hasOwn(payload, "branching")) {
    ...
    if (Object.hasOwn(branching, "naming")) {
      ...
      if (Object.hasOwn(naming, "max_slug_length") && (!Number.isInteger(naming.max_slug_length) || naming.max_slug_length < 12 || naming.max_slug_length > 100)) {
...
function validateProfileBaseRef(value, profileId) {
  const invalid = typeof value !== "string" || !value || value.startsWith("-") || value.startsWith("/") || value.endsWith("/") || value.endsWith(".") || value.endsWith(".lock") || value.includes("..") || value.includes("@{") || value.includes("//") || [...value].some((character) => /\s/u.test(character) || character.codePointAt(0) < 32) || /[~^:?*[\\]/u.test(value);
...
function profilePromptProjection(profile, phaseId) {
  ...
  if (Array.isArray(projection.profiles)) {
    projection.profiles = projection.profiles.map((value) => isMapping(value) ? profilePromptProjection(value, phaseId) : value);
  }
...
function trimPromptArchitecture(value) {
  return Object.fromEntries(
    Object.entries(value).filter(([key]) => !["roles", "forbidden_dependencies"].includes(key)).map(([key, child]) => [key, isMapping(child) ? trimPromptArchitecture(child) : child])
  );
}

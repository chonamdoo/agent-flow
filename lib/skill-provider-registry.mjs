...
function compareSemver(left, right) {
  const parse = (value) => String(value).split("-", 1)[0].split(".").map((part) => Number.parseInt(part, 10));
  const a = parse(left);
  const b = parse(right);
...
export function resolveSkillProviderClaims(registry, context, options = {}) {
  ...
  for (const rawCandidate of context.candidates) {
    ...
    if (eligible.length > 1) {
      if (new Set(eligible.map((provider) => semverMajor(provider.version))).size > 1) {
        quarantined.push(candidateDiagnostic(
          candidate,
          "version_conflict",
          new Error(`candidate has incompatible provider versions: ${
            eligible.map((provider) => `${provider.id}@${provider.version}`).sort(compareCodePoints).join(", ")
          }`),
        ));
        continue;
      }
      const ranked = [...eligible].sort(
        (left, right) => compareSemver(right.version, left.version) || compareCodePoints(left.id, right.id),
      );
      const top = ranked[0];
      if (ranked.filter((provider) => compareSemver(provider.version, top.version) === 0).length > 1) {
        quarantined.push(candidateDiagnostic(
          candidate,
      ...
      claims.push(providerClaim(candidate, top, registry.fingerprint, evidenceByProvider.get(top)));
      continue;
    }
    claims.push(providerClaim(
      candidate,

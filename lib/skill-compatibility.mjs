...
export function createSkillCompatibilityCatalog(value) {
  ...
  const capabilityProviders = () => {
    ...
    for (const [canonical, record] of byCanonical) {
    ...
    return new Map(
      [...providers].map(([cap, names]) => [cap, [...names].sort(compareCodePoints)]),
    );
  };
  return {
    projection,
    ...
    capabilityProviders,
    resolveCapability(capability, available) {
      const cap = compatibilityName(capability, "capability");
      const availableSet = new Set(
        [...logicalNameValues(available)].map((name) => compatibilityName(name, "available skill")),
      );
      const providers = capabilityProviders().get(cap) ?? [];
      const eligible = providers.filter((name) => availableSet.has(name));
      if (eligible.length === 0) {
        return { capability: cap, resolved: false, canonical: null, providers, reason: "capability_unresolved" };
      }
      if (eligible.length > 1) {
        return { capability: cap, resolved: false, canonical: null, providers: eligible, reason: "stack_ambiguity" };
      }
      return { capability: cap, resolved: true, canonical: eligible[0], providers: eligible, reason: null };
    },
  };
}

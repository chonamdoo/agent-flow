...
def skill_links_commitment(skill_plan_hash: object, links: object) -> str:
    ...
    payload = {
        ...
        "links": _normalized_skill_links(links),
    }
    return hashlib.sha256(
        json.dumps(
            payload,
...
def assert_committed_skill_host_links_applied(
    ...
    normalized_links = _normalized_skill_link_objects(links)
    ...
    if pinned_hash:
        if pinned_version == SKILL_PLAN_HASH_VERSION:
            if pinned_hash != current_hash:
                raise SkillPlanSnapshotError(
                    "blocked: installed skill plan changed during the active run; "
                    "restore the pinned snapshot or start a new run"
                )
            ...
            if pinned_local_hash:
                if (
                    ...
                    raise SkillPlanSnapshotError(
                        "blocked: project-local skill plan changed during the active run; "
